...
fn default_usage_provider_performance_include_timeline() -> bool {
    true
}
...
pub trait UsageReadRepository: Send + Sync {
    ...
    async fn summarize_dashboard_stats(
        &self,
        query: &UsageDashboardSummaryQuery,
    ) -> Result<StoredUsageDashboardStatsSummary, crate::DataLayerError> {
        let usage = self.summarize_dashboard_usage(query).await?;
    ...
    /// Reads the exact pending portion of usage-counter health for frequent observability.
    ///
    /// Implementations should avoid deriving this snapshot from processed-row totals: the
    /// processed outbox can be arbitrarily large, while pending rows are bounded by active
    /// backlog. The full, exact administrative snapshot remains available through
    /// [`Self::read_usage_counter_health`].
    async fn read_usage_counter_pending_health(
        &self,
    ) -> Result<UsageCounterPendingHealthSnapshot, crate::DataLayerError> {
        let snapshot = self.read_usage_counter_health().await?;
        Ok(UsageCounterPendingHealthSnapshot {
...
/// Exact pending usage-counter health suitable for frequent metrics collection.
///
/// This deliberately excludes processed-row totals and the latest processed timestamp. Those
/// fields require inspecting the unbounded processed history and belong to the lower-frequency
/// administrative health snapshot instead.
#[derive(Debug, Clone, PartialEq, Eq, Default, serde::Serialize, serde::Deserialize)]
pub struct UsageCounterPendingHealthSnapshot {
    pub pending_rows: u64,
    pub oldest_pending_created_at_unix_secs: Option<u64>,
    pub pending_by_kind: std::collections::BTreeMap<String, u64>,
}
...
mod tests {
    ...
    fn provider_performance_query_defaults_timeline_on_for_legacy_payloads() {
        ...
        let legacy: UsageProviderPerformanceQuery =
            serde_json::from_value(payload.clone()).expect("legacy query should deserialize");
        assert!(legacy.include_timeline);

        payload["include_timeline"] = Value::Bool(false);
        let without_timeline: UsageProviderPerformanceQuery = serde_json::from_value(payload)
            .expect("explicit timeline preference should deserialize");
        assert!(!without_timeline.include_timeline);
    }
