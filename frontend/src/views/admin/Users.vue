...
const userApiKeyForm = ref<UserApiKeyFormState>({
  ...
  billing_multiplier_mode: 'custom',
  billing_source: 'auto',
  ip_rules_text: '',
...
function openEditUserApiKeyDialog(apiKey: ApiKey) {
  ...
  userApiKeyForm.value = {
    ...
    billing_multiplier_mode: apiKey.billing_multiplier_mode ?? 'custom',
    billing_source: apiKey.billing_source ?? 'auto',
  ...
  try {
    ...
    if (editingApiKey) {
      await usersStore.updateApiKey(targetUserId, editingApiKey.id, {
        ...
        billing_multiplier_mode: form.billing_multiplier_mode,
